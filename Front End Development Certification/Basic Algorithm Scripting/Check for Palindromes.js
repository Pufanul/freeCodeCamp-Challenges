...
  //"start" and "finish" pointers won't always meet in the middle, so it's best suited to use (start > finish)
  //this will compare the first half of the string with the last half of the string to see if the current characters match
  while (start > finish) {    
 ...
    while ( str[start].match(/[\W_]/) ) {      
    ...
    if ( str[start].toLowerCase() !== str[finish].toLowerCase() ) return false;    
    start++;    
    finish--;  
  }    
